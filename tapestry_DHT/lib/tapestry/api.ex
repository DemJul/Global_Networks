...
defmodule Tapestry.API do
  ...
  def run_simulation(num_nodes, num_requests) do
    ...
    wait_for_all_requests_to_complete(num_nodes * num_requests, 0,0)
  end
  
  # Returns and prints the `max_hop_count` when `num_requests` are completed.
  defp wait_for_all_requests_to_complete(num_requests, max_hop_count,n) do
    if num_requests > 0 do
      receive do
        {:done, hop_count} ->
          ...
          n=n+hop_count
          wait_for_all_requests_to_complete(num_requests - 1, max_hop_count,n)
      end
    else
      IO.puts "All requests completed. Max hops needed = #{max_hop_count} Number of arcs of the graph = #{n}"
    end
  end
end
